...
function renderizarVaquinhas(vaquinhas) {
    var lista = document.getElementById('lista-campanhas');
    if (!lista) return;
    lista.innerHTML = '';

    var filtradas = vaquinhas.filter(function(v) {
        if (categoriaAtiva === 'todos') return true;
        return v.categoria === categoriaAtiva;
    });

    if (filtradas.length === 0) {
        lista.innerHTML = '<p style="text-align:center;color:#888;padding:2rem;">Nenhuma campanha encontrada nessa categoria.</p>';
        return;
    }
    ...
    for (var i = 0; i < filtradas.length; i++) {
        ...
        var dataTexto = v.dataLimite ? 'Até ' + v.dataLimite : '';
...
function ativarFiltros() {
    var botoes = document.querySelectorAll('[data-cat]');
    botoes.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            // Ativa todos os botões com o mesmo data-cat (chip + sidebar)
            document.querySelectorAll('[data-cat="' + categoriaAtiva + '"]').forEach(function(b) {
                b.classList.add('ativo');
            });

            renderizarVaquinhas(todasVaquinhas);
        });
    });
}
